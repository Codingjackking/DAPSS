...
class MetricsCollector:
    ...
    def __init__(self, node_id: str, metrics_dir: str = "metrics"):
        ...
        self.adaptive_file  = os.path.join(metrics_dir, f"adaptive_stats_{safe_id}.json")
        self.gossip_file    = os.path.join(metrics_dir, f"gossip_metrics_{safe_id}.json")
        self.consensus_file = os.path.join(metrics_dir, f"consensus_metrics_{safe_id}.json")
        self.network_file   = os.path.join(metrics_dir, f"network_health_{safe_id}.json")
        self.summary_file   = os.path.join(metrics_dir, f"summary_{safe_id}.json")
...
def initialize_metrics(node_id: str, metrics_dir: str = "metrics") -> MetricsCollector:
    """Return a new metrics collector for each node."""
    return MetricsCollector(node_id, metrics_dir)
